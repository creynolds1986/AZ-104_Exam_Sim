    ...
    subQuestions: [
      ...
      },
      {
        ...
        options: [
          ...
          { id: 'c', text: 'Inbound Deny rule: Source = Any, Destination port = Any, Protocol = Any (e.g. Allow at priority 100, Deny at priority 200)' },
          { id: 'd', text: 'Outbound Deny rule: Source = 10.2.0.0/16, Destination = Internet, Protocol = Any' },
        ],
        correctOptionIds: ['a', 'c'],
        requiredSelections: 2,
        explanation: 'Create an inbound Allow rule for SQL traffic on port 1433 from the web tier subnet (10.1.0.0/16) at a higher-priority number such as 100, and then add an inbound Deny-all rule at a lower-priority number such as 200. NSGs also include a default DenyAllInbound rule at priority 65500, but an explicit deny rule at priority 200 gives you defense-in-depth. The source should be the specific web tier CIDR, not "Any".',
      },
    ],
    ...
    subQuestions: [
      {
        ...
        correctOptionId: 'a',
        explanation: 'Assigning SQL DB Contributor at the rg-data resource group scope ensures the DBA team can manage only SQL Databases without granting access to Storage Accounts in the same resource group. The SQL DB Contributor role includes only SQL-related permissions, so it does not extend to Storage Accounts. Contributor at rg-data (option B) would give access to Storage Accounts, and SQL Server Contributor at the production subscription scope (option D) is too broad.',
      },
      {
        id: 'cs-007-q2',
        type: 'multiple-choice',
